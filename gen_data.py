...
def to_binary(value, num_bits):
    # little-endian
    return np.flipud(np.array([int(i) for i in np.binary_repr(value, num_bits)]))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate json file for given function.')
    parser.add_argument('numbits', type=int,
                        help='number of input bits per operand')
    parser.add_argument('function', metavar='function', choices=FUNCTIONS.keys(),
                        help='function to generate data for, one of: {}.'.format(
                            list(FUNCTIONS.keys())))
    parser.add_argument('--outfile', type=str, help=('output filename - if omitted will default '
