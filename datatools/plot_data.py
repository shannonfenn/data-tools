...
def count_generalised(df, N):
    target_names = ['test_err_tgt_{}'.format(i) for i in range(N)]
...
def get_results():
    filename = easygui.fileopenbox(msg='Select experiment file',
...
def plot_err(results, perbit, err_type, by, show_legend=True,
    ...
    if perbit:
        ...
        if err_type == 'test':
            ...
            target_names = ['trg_err_tgt_{}'.format(i) for i in range(N)]
        ...
        if by == 'lm':
            ...
            # aesthetic = gg.aes(x='grp', y='test_error', color='grp')
            aesthetic = gg.aes(x='test_error', y='grp', color='grp')
            x_label = 'Learner / Guiding Function'
            legend_name = 'Learner / Guiding Function'
        elif by == 'learner>gf':
            aesthetic = gg.aes(x='guiding_function', y='test_error', color='learner')
            x_label = 'Guiding Function'
            legend_name = 'Learner'
        elif by == 'gf>learner':
            aesthetic = gg.aes(x='learner', y='test_error', color='guiding_function')
            x_label = 'Learner'
            legend_name = 'Guiding Function'
        elif by == 'learner':
            aesthetic = gg.aes(x='learner', y='test_error', color='learner')
            x_label = 'Learner'
        elif by == 'gf':
            aesthetic = gg.aes(x='guiding_function', y='test_error', color='guiding_function')
            x_label = 'Guiding Function'
        elif by == 'tf':
            aesthetic = gg.aes(x='tfs', y='test_error', color='tfs')
...
#     # Test Error (simple) plot
#     plot_test_err = gg.ggplot(results, gg.aes(x=guiding_function, y=test_error, color=guiding_function)) +\
#         geom_boxplot(notch=use_notch) +\
...
#     # training error
#     plot_trg_err = gg.ggplot(results, gg.aes(x=guiding_function, y=trg_error, color=guiding_function)) +\
#         geom_boxplot() +\
...
#     plot_trg_err = plot_trg_err + theme(legend_position='none')
...
#     print(plot_trg_err, vp=vplayout(2, 2))
...
def plot_errs_by_sample(results):
    p_test = gg.ggplot(gg.aes(x=num.Ne, y=test_error, color=guiding_function), data=results) +\
        geom_boxplot() +\
        gg.labs(title='Test error', x='Ne', y='error')

    p_training = gg.ggplot(gg.aes(x=num.Ne, y=trg_error, color=guiding_function), data=results) +\
        geom_boxplot() +\
...
def plot_single_comparison(results):
    ...
    p_test = gg.ggplot(gg.aes(x=opt_name, y=test_error, color=opt_name), data=results) +\
        geom_boxplot(notch=True) +\
        gg.labs(title='Test error', x='optimiser', y='error') +\
        scale_color_discrete(name='Optimiser')

    p_training = gg.ggplot(gg.aes(x=opt_name, y=trg_error, color=opt_name), data=results) +\
        geom_boxplot(notch=False) +\
        gg.labs(title='Training error', x='optimiser', y='error') +\
        theme(axis.title.y=element_blank())

    p_iterations = gg.ggplot(gg.aes(x=opt_name, y=total_iterations, color=opt_name), data=results) +\
        geom_boxplot(notch=True) +\
